...
function sym(args) {
	let arrOfSets = [...arguments];
	function findSymDiffOfTwoSets(arr1, arr2) {
		let unique = new Set();
		arr1.forEach((num) => {
			if (!arr2.includes(num) && !unique.has(num)) {
				unique.add(num);
			}
		});

		arr2.forEach((num) => {
			if (!arr1.includes(num) && !unique.has(num)) {
				unique.add(num);
			}
		});
		return [...unique];
	}
	let result = arrOfSets.reduce(findSymDiffOfTwoSets);
	return result;
}
